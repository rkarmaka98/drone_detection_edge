...
while True:
    ...
    for i, detection_list in enumerate(model.predict([img], callback=fomo_post_process)):
        ...
        for (x, y, w, h), score in detection_list:
            ...
            # Prepare label text
            label = "%s: %.2f" % (model.labels[i], score)

            # Calculate text position (ensure it stays on screen)
            text_x = max(0, x)
            text_y = max(0, y - 10)

            # Draw label background
            img.draw_rectangle(text_x, text_y, len(label)*8, 10, color=colors[i], fill=True)

            # Draw label text (using simple draw_string without font object)
            img.draw_string(text_x + 2, text_y + 1, label, color=(0, 0, 0), scale=1)
